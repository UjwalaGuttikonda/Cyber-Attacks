...
def bobs_custom_pwhash_attack(pc, char_list, length, pwlist_obj):
    results = [None for _ in range(len(pwlist_obj))]
    assert (length % 2 == 0), 'Length must be a multiple of 2'

    import operator

    def get_hash_parts(pwhash):
        pwhash_length = len(pwhash)
        part_length = pwhash_length // 3
        return pwhash[:part_length], pwhash[part_length:2 * part_length], pwhash[2 * part_length:]

    def pwhash_fn(p, salt):
        return crypto.truncated_sha2_hex_digest(p + '-' + salt, 6)

    hash_p1, hash_p2, salts, pwhashs = [], [], [], []

    for pwitem in pwlist_obj:
        user = pwitem['user']
        salt = pwitem['salt']
        pwhash = pwitem['hash']

        p1, p2, salt_c = get_hash_parts(pwhash)
        hash_p1.append(p1)
        hash_p2.append(p2)
        salts.append(salt)
    ...
    def attack_part(pw_gen, hash_part, pwhash_fn, salt):
        return simple_exhaustive_attack(pc, pw_gen, lambda p: pwhash_fn(p, salt), hash_part)

    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        futures = [
            executor.submit(attack_part, pw_gen_1, hash_p1, pwhash_fn, 'p1'),
            executor.submit(attack_part, pw_gen_2, hash_p2, pwhash_fn, 'p2')
        ]

        for i, future in enumerate(concurrent.futures.as_completed(futures)):
            results[i] = future.result()

    # Replace None values with empty strings before concatenating
    results = ["" if result is None else result for result in results]

    results = list(map(operator.add, results[0], results[1]))

    for i in range(len(pwlist_obj)):
        password, salt, pwhash = results[i], salts[i], pwhashs[i]
        # assert pwhash_fn(password, salt) == pwhash
